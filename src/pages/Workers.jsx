...
const emptyForm = {
  ...
  work_type: "دوام كامل", salary: "", company_id: "", status: "نشط",
...
export default function Workers() {
  ...
  const handleSave = async () => {
    if (!form.full_name || !form.national_id || !form.company_id) {
      toast({ title: "خطأ", description: "الاسم ورقم الهوية والمنشأة مطلوبة", variant: "destructive" });
      return;
    }
  ...
  const filtered = workers.filter((w) =>
    w.full_name?.includes(search) || w.national_id?.includes(search) || w.job_title?.includes(search)
  );
  ...
  return (
    <div>
      <PageHeader title="العاملون عن بُعد" description="إدارة بيانات الموظفين العاملين عن بُعد">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={Users} title="لا يوجد موظفون" description="ابدأ بإضافة موظف جديد" />
      ) : (
              ...
              <tbody className="divide-y">
                {filtered.map((w) => (
                  <tr key={w.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4 text-muted-foreground">{getCompanyName(w.company_id)}</td>
